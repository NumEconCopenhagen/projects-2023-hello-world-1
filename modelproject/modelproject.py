...
class OLGModelClass():
    ...
    def setup(self):
        ...
        par.pop_young = [par.pop_initial] + [0] * (par.simT - 1)
        par.pop_old = [par.pop_initial*par.p[0]/(1+par.n)] + [0] * (par.simT - 1)
    ...
    def update_population(self, t):
        """ update the young and old population at time t """

        par = self.par

        # a. new young population is old young population increased by growth rate
        par.pop_young[t] = par.pop_young[t-1] * (1 + par.n)

        # b. new old population is surviving young population
        par.pop_old[t] = par.pop_young[t-1] * par.p[t-1]
...
def simulate_before_s(par,sim,t):
    ...
    if par.production_function == 'ces':
        ...
        # i. production
        sim.Y[t] = ( par.alpha*sim.K_lag[t]**(-par.theta) + (1-par.alpha)*((1+par.n)**t)**(-par.theta) )**(-1.0/par.theta)
